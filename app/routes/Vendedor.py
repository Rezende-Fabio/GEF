...
@vendedorBlue.route("/lista-vendedores", methods=["GET"])
@login_required
def listaVendedores():
    ###################################################################################################
...
@vendedorBlue.route("/vendedor/<id>", methods=["GET"])
@login_required
def popupViewVend(id):
    ...
    try:
        ...
        #Query que trás o vendedor que foi selcionado
        vendedor = conexao.query.get(id)
        context = {"vendedor": vendedor} #Dicionário contendo as variáveis para utilizar no template
        return render_template("vendedor/corpoModalVisuVend.html", context=context)
...
@vendedorBlue.route("/atualiza-baseVend", methods=["POST"])
@login_required
def atualizaBaseVend():
    ...
    # RETORNOS:
    #   return jsonify("success") = Retorna Json com sucesso;
    #   return jsonify("Error") = Retorna Json com erro.
    ###################################################################################################
    
    try:
        Atualizacao.integraVendedor() #Chama função para a atualização
        flash("Base atualizada com sucesso!", "success") #Mensagem para ser exibida no Front
...
def vendedores():
    ...
    try:
        if request.method == "POST":
            ...
            lista = []
            for x in vendedores:
                lista.append({"cod": x.v_id, "nome": x.v_nome, "cpfCnpj": filtroCpf(x.v_cpfcnpj), "tel": x.v_telefone, "status": filtroStatus(x.v_ativo)})  
            return jsonify(lista)
